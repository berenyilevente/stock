...
const Delivery: React.FC<DeliveryProps> = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    dispatch(postStockHeadAction(form.ugyfelid, false));
  ...
  return (
    <div className="mx-96">
      <div className="flex justify-between">
        <h1 className="text-3xl mx-32 my-12">{t("delivery.delivery")}</h1>
        <Button className="my-12" onClick={() => setOpenUploadModal(true)}>
          {t("delivery.addStockHead")}
        </Button>
      </div>
      <div className="grid justify-items-center">
        <Table
          tableData={stockHeadsDelivery}
          tableTitle={t("delivery.stockHeads")}
          totalCount={stockHeadsDelivery.length}
